...
`search(TAB_INDEX, DEST, SOURCES, CRITERIA, PASSCODE, ZONE_INDEX)`
...
| DEST | The zone index that searched cards should be moved to. For standard behaviour (revealing cards upon clicking "Spawn" instead of sending them to any specific location), set this parameter to 255.|
| SOURCES | An array of integers, each representing all the available tab indexes that should be shown to the player after the "Search Deck Piles" menu is opened. |
| CRITERIA | A string representing criteria by which the menu should filter out certain cards. Start this string with "type:" to filter by monster type, or "attr:" to filter by attribute. Otherwise, the system will only show cards containing the string you provide. If you provide an empty string (`\"\"`), the system will not filter the card list, and just show all the cards at the selected location. |
| PASSCODE | If a card's script should run after a single card is searched, provide this value with its passcode.<br/>If you instead want the system to attempt to run the script for "whichever single card was searched", set this parameter to -1. If you do not wish to use this feature, set this parameter to 0. |
| ZONE_INDEX | If you set PASSCODE to a value other than 0, then set this parameter to the zone index that you'd like the desired card script to trigger from. For example, if PASSCODE is the passcode of a spell card, or if you set PASSCODE to -1 and expect the player to only search spell cards, then you can set ZONE_INDEX to 22 to have the system run the desired cards "On-Snapped-To-Spell/Trap-Zone" effect. |
