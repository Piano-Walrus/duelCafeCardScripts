...
| SOURCES | int[] | Representing all the available tab indexes that should be shown to the player after the "Search Deck Piles" menu is opened. If the only array element is 255, then the only available tab shown to the user will be the one provided for the TAB_INDEX parameter. |
...
If only one card is searched from the "Search Deck Piles" menu using the `search` function, then after the `then` keyword, you can use a few extra placeholders to get information about that searched card. These placeholders can also be used after a card is hit using the `excavate_until` function, in which case the `then` keyword is not required. Note that these work the same way as the card properties outlined in the [Logical Expressions](https://github.com/Piano-Walrus/duelCafeCardScripts/blob/main/README.md#logical-expressions) section:
...
| `prev_range`  | int[]  | Similar to `prev_zone`, except the system checks every zone index from the provided array's first index to its last index (inclusive). This property expects an array with only two values, signifying a "start index" and "end index." Also, note that when using this property, the card's previous zone will always be below 100. |
| `prev_list`  | int[]  | Similar to `prev_zone`, except the system checks every zone index in the provided array. Also, note that when using this property, the card's previous zone will always be below 100. |
...
Alternatively, you may also use JavaScript-style variable declaration syntax to declare custom properties by renaming a script file to end with the ".js" extension. However, note that when referencing each variable, preceding each variable name with `$` is still required. A couple good examples of this alternate syntax would be [Radiant Typhoon Eldam](https://github.com/Piano-Walrus/duelCafeCardScripts/blob/main/Scripts/0-Main/54143349.js) and [Sky Striker Ace = Zero](https://github.com/Piano-Walrus/duelCafeCardScripts/blob/main/Scripts/1-Extra/76072561.js).
